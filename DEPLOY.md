...
- `/workflow` 是当前实例的只读 workflow-stage 配置可视化页面，读取该实例的 `WORKFLOW.md`
  ...
  `current_stage` 运行态分布；桌面端的 Mermaid 图和当前阶段检查器同屏展示，便于点击节点后立即看到阶段详情。首屏使用紧凑状态条展示入口阶段、阶段数量、结束阶段、运行快照和诊断提醒；如果存在 unreachable stage、tracker 映射缺失等提醒，状态条会直接显示第一条提醒摘要并链接到“配置诊断”，顶部状态会显示“配置有提醒”。页面不展示 token、`api_key` 或 env secret 原始值。
- `/admin/instances` 从 `~/.config/symphony/projects` 发现已登记实例，聚合 systemd user service 状态和各实例 `/api/v1/state`。
- 实例总览在页面首屏之后后台加载，并带有总超时和单实例探测隔离；加载完成前，创建、自动更新、timer 等写操作会先禁用并指向可见原因，避免在实例健康未知时执行高影响动作。慢实例会显示成该实例自己的不可达/未知健康状态，不会让整个管理页或列表 API 长时间无响应。
