...
- `/workflow` 是当前实例的只读 workflow-stage 配置可视化页面，读取该实例的 `WORKFLOW.md`
  ...
  `current_stage` 运行态分布；桌面端的 Mermaid 图和当前阶段检查器同屏展示，便于点击节点后立即看到阶段详情。Mermaid 节点支持键盘聚焦，并会在聚焦时给出可见描边。首屏使用紧凑状态条展示入口阶段、阶段数量、结束阶段、运行快照和诊断提醒；如果存在 unreachable stage、tracker 映射缺失等提醒，状态条会先显示中文行动摘要并链接到“配置诊断”，顶部状态会显示“配置有提醒”。页面不展示 token、`api_key` 或 env secret 原始值。
- `/admin/instances` 从 `~/.config/symphony/projects` 发现已登记实例，聚合 systemd user service 状态和各实例 `/api/v1/state`。
- 实例总览在页面首屏之后后台加载，并带有总超时和单实例探测隔离；实例、GitHub main 自动更新、systemd timer 使用独立加载通道展示，加载完成前写操作会先禁用并指向可见原因，避免在实例健康未知时执行高影响动作。Fleet 摘要会把“不可达/未知实例”放在最前，并把运行中、重试中、阻塞数标成“可达实例中...”的计数；不可达实例不会被当作 0 风险。慢实例会显示成该实例自己的不可达/未知健康状态，不会让整个管理页或列表 API 长时间无响应。
...
- 管理页按钮还会按当前状态降级：禁用按钮使用中性灰而不是继续保留主操作/危险色；自动更新不可用或没有可执行更新时不会把“执行更新”表现为可用；`symphony-update.timer` 已启用时不会把“启用”表现为可用，已禁用时不会把“禁用”表现为可用；legacy unit 已归档或 `not-found` 时实例 lifecycle/log 操作会禁用并显示原因。远程只读浏览器不能打开新建实例表单；新建实例表单会在本机提交安装脚本前显示字段级错误和错误摘要，避免无效输入直接进入 systemd/install 流程。
