...
部署侧只把它作为 Owner 关闭决策证据索引，不新增服务、unit、systemd 操作、provider 调用、worker
starter、writeback、workspace hook、配置变更或 legacy service takeover。
...
- `/` 是当前进程的 Hub 运行总览，首屏优先展示当前需要关注的工作和 Hub 项目焦点；Token 总数、运行时长等消耗指标放在独立的“运行消耗”区域，不再和运行中/Hub/阻塞/重试摘要重复。Hub 项目焦点会把活跃尝试、workspace lease、manual attention 和下一步 project action 放在 Hub 设备诊断前。下一步 action 会转换成面向操作员的人话说明并独占一行，同时写清项目名、attempt/workspace lease/lifecycle 数、Hub 项目明细证据入口，以及“当前页只读 / 需本机管理员到实例管理或命令行处理”等边界，避免把只读观察误解成可直接操作；焦点行和“查看 Hub 活跃尝试”会直接锚到对应项目行，浏览器会自动展开 Hub 项目明细并聚焦目标行。如果运行中 Issue 为 0 但 Hub 仍有活跃尝试，首屏链接会跳到对应 Hub 项目，而不是跳到空的运行明细。页面会标出当前访问身份、首次快照是否已加载，以及该页是只读观测面。空的速率限制快照不显示，完整 Hub 诊断矩阵和 Hub 项目明细默认收在可展开的高级诊断区域里。
- `/workflow` 是当前实例的只读 workflow-stage 配置可视化页面，读取该实例的 `WORKFLOW.md`
  ...
  `current_stage` 运行态分布；桌面端的 Mermaid 图和当前阶段检查器同屏展示，便于点击节点后立即看到阶段详情。Mermaid 节点支持键盘聚焦，并会在聚焦时给出可见描边。首屏使用紧凑状态条展示入口阶段、阶段数量、结束阶段、运行快照和诊断提醒；如果存在 unreachable stage、tracker 映射缺失等提醒，状态条会先显示中文行动摘要，并提供“查看受影响阶段”和“配置诊断”两个入口。可滚动 prompt 预览会标为只读预览区域。页面不展示 token、`api_key` 或 env secret 原始值。
- `/admin/instances` 从 `~/.config/symphony/projects` 发现已登记实例，聚合 systemd user service 状态和各实例 `/api/v1/state`。
- 实例总览在页面首屏之后后台加载，并带有总超时和单实例探测隔离；实例、GitHub main 自动更新、systemd timer 使用独立加载通道展示，加载完成前写操作会先禁用并指向可见原因，加载失败且没有已确认实例总览时写操作继续锁定，避免在实例健康未知时执行高影响动作。页面会在首屏明确区分当前管理控制台和被管理的项目实例端口；列表里的 stopped/不可达只代表对应 `symphony@...` 项目实例，不代表当前控制台停止。Fleet 摘要会把“不可达/未知实例”放在最前，并把运行中、重试中、阻塞数标成“可达实例中...”的计数；不可达实例卡片的 Issue 压力显示“未取到快照 / Issue 数未知”，不会被当作 0 风险。慢实例会显示成该实例自己的不可达/未知健康状态，不会让整个管理页或列表 API 长时间无响应。
- 页面顶部固定提供“Hub 运行总览 / 流程配置 / 实例管理”三类入口，并标出当前访问身份是“本机管理员”还是“远程只读”。
- 每个 `symphony@<project>.service` 仍然独立拥有自己的 `WORKFLOW.md`、`TRACKER.yaml`、环境变量、日志目录、workspace root、端口和内存调度账本。
- 停止、失败或 API 不可达的实例会显示为该实例自己的健康状态，不会影响其他实例展示；这类实例的 Dashboard/API 入口会降级成带可见原因的不可点击文本，仍保留 URL 方便排查，避免点到不可用或远程浏览器上的 `127.0.0.1`。如果实例没有配置 URL，不额外渲染假的禁用入口，只保留未配置端口/入口提示。
...
- 管理页按钮还会按当前状态降级：禁用按钮使用中性灰而不是继续保留主操作/危险色；自动更新不可用或没有可执行更新时不会把“执行更新”表现为可用；`symphony-update.timer` 已启用时不会把“启用”表现为可用，已禁用时不会把“禁用”表现为可用；如果被管理实例存在不可达或未知，timer 启用/禁用/手动触发的确认文案会带上“Issue 数可能未知”的风险事实；legacy unit 已归档或 `not-found` 时实例 lifecycle/log 操作会禁用并显示原因。远程只读浏览器不能打开新建实例表单；新建实例表单会在本机提交安装脚本前显示字段级错误和错误摘要，避免无效输入直接进入 systemd/install 流程。
