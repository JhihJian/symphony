...
- 实例总览在页面首屏之后后台加载，并带有总超时和单实例探测隔离；实例、GitHub main 自动更新、systemd timer 使用独立加载通道展示，加载完成前写操作会先禁用并指向可见原因，避免在实例健康未知时执行高影响动作。页面会在首屏明确区分当前管理控制台和被管理的项目实例端口；列表里的 stopped/不可达只代表对应 `symphony@...` 项目实例，不代表当前控制台停止。Fleet 摘要会把“不可达/未知实例”放在最前，并把运行中、重试中、阻塞数标成“可达实例中...”的计数；不可达实例不会被当作 0 风险。慢实例会显示成该实例自己的不可达/未知健康状态，不会让整个管理页或列表 API 长时间无响应。
