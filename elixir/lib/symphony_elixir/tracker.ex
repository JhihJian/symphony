defmodule SymphonyElixir.Tracker do
  ...
  defp validate_known_provider_states(tracker_config, opts) do
    ...
    if MapSet.size(known_states) == 0 do
      :ok
    else
      tracker_config
      |> TrackerConfig.stage_states()
      |> Enum.flat_map(fn {stage_id, %{"state" => state}} ->
        if MapSet.member?(known_states, normalize_provider_state(state)) do
          []
        else
          [{stage_id, state}]
        end
  ...
  @spec unknown_provider_state_result([{String.t(), String.t()}], MapSet.t()) :: validation_result()
  defp unknown_provider_state_result([], _known_states), do: :ok

  defp unknown_provider_state_result(unknown_states, known_states) do
    details =
      unknown_states
      |> Enum.map(fn {stage_id, state} -> "#{stage_id}=#{inspect(state)}" end)
      |> Enum.join(", ")
  ...
  @spec normalize_provider_state_set(term()) :: MapSet.t()
  ...
  defp normalize_provider_state_set(_states), do: MapSet.new()
