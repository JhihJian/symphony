defmodule SymphonyElixir.Hub.DeviceObservability do
  ...
  def build(sources, opts) when is_map(sources) and is_list(opts) do
    ...
    cutover_authorization_consumption_guard =
      source_map(sources, [:cutover_authorization_consumption_guard, :hub_cutover_authorization_consumption_guard])

    cutover_execution_outcome_ledger =
      source_map(sources, [:cutover_execution_outcome_ledger, :hub_cutover_execution_outcome_ledger])

    cutover_execution_outcome_closeout =
      source_map(sources, [:cutover_execution_outcome_closeout, :hub_cutover_execution_outcome_closeout])

    cutover_replay_decision =
      source_map(sources, [:cutover_replay_decision, :hub_cutover_replay_decision])
    ...
    project_ids =
      [
        ...
        cutover_authorization_consumption_guard |> list_value(:projects) |> Enum.map(&required_string(&1, :project_id)),
        cutover_execution_outcome_ledger |> list_value(:projects) |> Enum.map(&required_string(&1, :project_id)),
  ...
  def to_snapshot(projection) when is_map(projection) do
    ...
    cutover_authorization_consumption_guard =
      cutover_authorization_consumption_guard_snapshot(
        value(projection, :cutover_authorization_consumption_guard),
        projects,
        generated_at
      )

    projects =
      attach_project_cutover_authorization_consumption_guards(
        projects,
        cutover_authorization_consumption_guard.projects
      )
    ...
    overview =
      Map.put(
        overview,
        :cutover_authorization_consumption_guard,
        cutover_authorization_consumption_guard_overview_snapshot(cutover_authorization_consumption_guard)
      )

    cutover_execution_outcome_ledger =
      cutover_execution_outcome_ledger_snapshot(
        value(projection, :cutover_execution_outcome_ledger),
        projects,
        generated_at
      )

    projects =
      attach_project_cutover_execution_outcome_ledgers(
        projects,
        cutover_execution_outcome_ledger.projects
      )
    ...
    overview =
      Map.put(
        overview,
        :cutover_execution_outcome_ledger,
        cutover_execution_outcome_ledger_overview_snapshot(cutover_execution_outcome_ledger)
      )

    cutover_execution_outcome_closeout =
      cutover_execution_outcome_closeout_snapshot(
        value(projection, :cutover_execution_outcome_closeout),
        projects,
        cutover_execution_outcome_ledger,
        generated_at
      )
    ...
    projects =
      attach_project_cutover_execution_outcome_closeouts(
        projects,
        cutover_execution_outcome_closeout.projects
      )
    ...
    overview =
      Map.put(
        overview,
        :cutover_execution_outcome_closeout,
        cutover_execution_outcome_closeout_overview_snapshot(cutover_execution_outcome_closeout)
      )

    cutover_replay_decision =
      cutover_replay_decision_snapshot(
        value(projection, :cutover_replay_decision),
        projects,
        generated_at
      )

    projects =
      attach_project_cutover_replay_decisions(
        projects,
        cutover_replay_decision.projects
      )
    ...
    overview =
      Map.put(
        overview,
        :cutover_replay_decision,
        cutover_replay_decision_overview_snapshot(cutover_replay_decision)
      )

    cutover_replay_request_audit =
      cutover_replay_request_audit_snapshot(
        value(projection, :cutover_replay_request_audit),
        projects,
        cutover_execution_outcome_ledger,
        cutover_execution_outcome_closeout,
        cutover_replay_decision,
    ...
    projects =
      attach_project_cutover_replay_request_audits(
        projects,
        cutover_replay_request_audit.projects
      )
    ...
    cutover_closure_conclusion =
      cutover_closure_conclusion_snapshot(
        value(projection, :cutover_closure_conclusion),
        cutover_closure_chain,
        generated_at
      )

    projects =
      attach_project_cutover_closure_conclusions(
        projects,
        cutover_closure_conclusion.projects
      )
  ...
  defp project_snapshot(project) when is_map(project) do
    ...
    cutover_authorization_consumption_guard =
      cutover_authorization_consumption_guard_project_snapshot(value(project, :cutover_authorization_consumption_guard))

    cutover_execution_outcome_ledger =
      cutover_execution_outcome_ledger_project_snapshot(value(project, :cutover_execution_outcome_ledger))

    cutover_execution_outcome_closeout =
      cutover_execution_outcome_closeout_project_snapshot(value(project, :cutover_execution_outcome_closeout))

    cutover_replay_decision =
      cutover_replay_decision_project_snapshot(value(project, :cutover_replay_decision))
  ...
  defp overview_snapshot(overview, projects) do
    ...
    cutover_authorization_consumption_guard =
      overview
      |> value(:cutover_authorization_consumption_guard)
      |> cutover_authorization_consumption_guard_overview_snapshot()

    cutover_execution_outcome_ledger =
      overview
      |> value(:cutover_execution_outcome_ledger)
      |> cutover_execution_outcome_ledger_overview_snapshot()

    cutover_execution_outcome_closeout =
      overview
      |> value(:cutover_execution_outcome_closeout)
      |> cutover_execution_outcome_closeout_overview_snapshot()

    cutover_replay_decision =
      overview
      |> value(:cutover_replay_decision)
      |> cutover_replay_decision_overview_snapshot()
  ...
  defp cutover_authorization_consumption_guard_snapshot(guard, projects, generated_at) do
    project_guards =
      projects
      |> Enum.map(&value(&1, :cutover_authorization_consumption_guard))
      |> Enum.filter(&is_map/1)

    cond do
      is_map(guard) and (list_value(guard, :projects) != [] or list_value(guard, :recent_decisions) != []) ->
        CutoverAuthorizationConsumptionGuard.to_snapshot(guard)

      project_guards != [] ->
        CutoverAuthorizationConsumptionGuard.to_snapshot(%{
          generated_at: generated_at,
          projects: project_guards
        })

      true ->
        CutoverAuthorizationConsumptionGuard.build(%{generated_at: generated_at}, now: generated_at)
    end
  end

  defp cutover_authorization_consumption_guard_project_snapshot(nil), do: nil

  defp cutover_authorization_consumption_guard_project_snapshot(guard) when is_map(guard) do
    summary = CutoverAuthorizationConsumptionGuard.to_snapshot(%{projects: [guard]})
    Enum.find(summary.projects, &(required_string(&1, :project_id) == required_string(guard, :project_id)))
  end

  defp cutover_authorization_consumption_guard_project_snapshot(_guard), do: nil

  defp attach_project_cutover_authorization_consumption_guards(projects, guards) do
    guards_by_project =
      guards
      |> Enum.reject(&is_nil/1)
      |> Map.new(&{&1.project_id, &1})
    ...
    Enum.map(projects, fn project ->
      Map.put(project, :cutover_authorization_consumption_guard, Map.get(guards_by_project, project.project_id))
    end)
  end

  defp cutover_execution_outcome_ledger_snapshot(ledger, projects, generated_at) do
    project_ledgers =
      projects
      |> Enum.map(&value(&1, :cutover_execution_outcome_ledger))
      |> Enum.filter(&is_map/1)

    cond do
      is_map(ledger) and (list_value(ledger, :projects) != [] or list_value(ledger, :recent_outcomes) != []) ->
        CutoverExecutionOutcomeLedger.to_snapshot(ledger)
      ...
      true ->
        CutoverExecutionOutcomeLedger.build(%{generated_at: generated_at}, now: generated_at)
    end
  end

  defp cutover_execution_outcome_ledger_project_snapshot(nil), do: nil

  defp cutover_execution_outcome_ledger_project_snapshot(ledger) when is_map(ledger) do
    summary = CutoverExecutionOutcomeLedger.to_snapshot(%{projects: [ledger]})
    Enum.find(summary.projects, &(required_string(&1, :project_id) == required_string(ledger, :project_id)))
  end

  defp cutover_execution_outcome_ledger_project_snapshot(_ledger), do: nil

  defp attach_project_cutover_execution_outcome_ledgers(projects, ledgers) do
    ledgers_by_project =
      ledgers
      |> Enum.reject(&is_nil/1)
      |> Map.new(&{&1.project_id, &1})
    ...
    Enum.map(projects, fn project ->
      Map.put(project, :cutover_execution_outcome_ledger, Map.get(ledgers_by_project, project.project_id))
    end)
  end

  defp cutover_execution_outcome_closeout_snapshot(closeout, projects, cutover_execution_outcome_ledger, generated_at) do
    project_closeouts =
      projects
      |> Enum.map(&value(&1, :cutover_execution_outcome_closeout))
      |> Enum.filter(&is_map/1)

    cond do
      is_map(closeout) and (list_value(closeout, :projects) != [] or list_value(closeout, :recent_closeouts) != []) ->
        CutoverExecutionOutcomeCloseout.to_snapshot(closeout)

      project_closeouts != [] ->
        CutoverExecutionOutcomeCloseout.to_snapshot(%{
          generated_at: generated_at,
          projects: project_closeouts
        })
  ...
  defp cutover_execution_outcome_closeout_project_snapshot(nil), do: nil

  defp cutover_execution_outcome_closeout_project_snapshot(closeout) when is_map(closeout) do
    summary = CutoverExecutionOutcomeCloseout.to_snapshot(%{projects: [closeout]})
    Enum.find(summary.projects, &(required_string(&1, :project_id) == required_string(closeout, :project_id)))
  end

  defp cutover_execution_outcome_closeout_project_snapshot(_closeout), do: nil

  defp attach_project_cutover_execution_outcome_closeouts(projects, closeouts) do
    closeouts_by_project =
      closeouts
      |> Enum.reject(&is_nil/1)
      |> Map.new(&{&1.project_id, &1})

    Enum.map(projects, fn project ->
      closeout = Map.get(closeouts_by_project, project.project_id)

      project
      |> Map.put(:cutover_execution_outcome_closeout, closeout)
      |> put_project_detail_outcome_closeout(closeout)
    end)
  end

  defp put_project_detail_outcome_closeout(project, nil), do: project

  defp put_project_detail_outcome_closeout(project, closeout) do
    detail =
      project
      |> value(:detail)
      |> case do
      ...
      |> Map.put(:outcome_closeout, project_closeout_detail(closeout, optional_string(project, :project_id)))

    Map.put(project, :detail, detail)
  end

  defp cutover_replay_decision_snapshot(decision, projects, generated_at) do
    project_decisions =
      projects
      |> Enum.map(&value(&1, :cutover_replay_decision))
      |> Enum.filter(&is_map/1)

    cond do
      is_map(decision) and (list_value(decision, :projects) != [] or list_value(decision, :recent_decisions) != []) ->
        CutoverReplayDecision.to_snapshot(decision)

      project_decisions != [] ->
        CutoverReplayDecision.to_snapshot(%{
          generated_at: generated_at,
          projects: project_decisions
        })

      true ->
        CutoverReplayDecision.to_snapshot(%{generated_at: generated_at})
    end
  end

  defp cutover_replay_decision_project_snapshot(nil), do: nil

  defp cutover_replay_decision_project_snapshot(decision) when is_map(decision) do
    summary = CutoverReplayDecision.to_snapshot(%{projects: [decision]})
    Enum.find(summary.projects, &(required_string(&1, :project_id) == required_string(decision, :project_id)))
  end

  defp cutover_replay_decision_project_snapshot(_decision), do: nil

  defp attach_project_cutover_replay_decisions(projects, decisions) do
    ...
    Enum.map(projects, fn project ->
      decision = Map.get(decisions_by_project, project.project_id)

      project
      |> Map.put(:cutover_replay_decision, decision)
      |> put_project_detail_replay_decision(decision)
    end)
  end

  defp put_project_detail_replay_decision(project, nil), do: project

  defp put_project_detail_replay_decision(project, decision) do
    detail =
      project
      |> value(:detail)
      |> case do
      ...
      |> Map.put(:replay_decision, project_replay_decision_detail(decision, optional_string(project, :project_id)))

    Map.put(project, :detail, detail)
  end

  defp cutover_replay_request_audit_snapshot(
         audit,
         projects,
         cutover_execution_outcome_ledger,
         cutover_execution_outcome_closeout,
    ...
    project_audits =
      projects
      |> Enum.map(&value(&1, :cutover_replay_request_audit))
      |> Enum.filter(&is_map/1)

    cond do
      is_map(audit) and (list_value(audit, :projects) != [] or list_value(audit, :recent_requests) != []) ->
        CutoverReplayRequestAudit.to_snapshot(audit)

      project_audits != [] ->
        CutoverReplayRequestAudit.to_snapshot(%{
          generated_at: generated_at,
          projects: project_audits
        })
  ...
  defp cutover_replay_request_audit_project_snapshot(nil), do: nil

  defp cutover_replay_request_audit_project_snapshot(audit) when is_map(audit) do
    summary = CutoverReplayRequestAudit.to_snapshot(%{projects: [audit]})
    Enum.find(summary.projects, &(required_string(&1, :project_id) == required_string(audit, :project_id)))
  end

  defp cutover_replay_request_audit_project_snapshot(_audit), do: nil

  defp attach_project_cutover_replay_request_audits(projects, audits) do
    audits_by_project =
      audits
      |> Enum.reject(&is_nil/1)
      |> Map.new(&{&1.project_id, &1})
    ...
    Enum.map(projects, fn project ->
      audit = Map.get(audits_by_project, project.project_id)

      project
      |> Map.put(:cutover_replay_request_audit, audit)
      |> put_project_detail_replay_request_audit(audit)
    end)
  end

  defp put_project_detail_replay_request_audit(project, nil), do: project

  defp put_project_detail_replay_request_audit(project, audit) do
    detail =
      project
      |> value(:detail)
      |> case do
  ...
  defp cutover_closure_conclusion_snapshot(conclusion, closure_chain, generated_at) do
    if usable_closure_conclusion?(conclusion) do
      CutoverClosureConclusion.to_snapshot(conclusion)
    else
      CutoverClosureConclusion.build(closure_chain, now: generated_at)
    end
  end

  defp usable_closure_conclusion?(conclusion) when is_map(conclusion) do
    list_value(conclusion, :projects) != [] or
      list_value(conclusion, :recent_chains) != [] or
      not blank?(optional_string(conclusion, :conclusion)) or
      not blank?(optional_string(conclusion, :summary_code))
  end

  defp usable_closure_conclusion?(_conclusion), do: false

  defp cutover_closure_conclusion_project_snapshot(nil), do: nil

  defp cutover_closure_conclusion_project_snapshot(conclusion) when is_map(conclusion) do
    closure_conclusion_project_summary(conclusion)
  end

  defp cutover_closure_conclusion_project_snapshot(_conclusion), do: nil

  defp attach_project_cutover_closure_conclusions(projects, conclusions) do
    conclusions_by_project =
      conclusions
      |> Enum.reject(&is_nil/1)
      |> Map.new(&{&1.project_id, &1})
    ...
    Enum.map(projects, fn project ->
      conclusion =
        ...
        |> cutover_closure_conclusion_project_snapshot()

      project
      |> Map.put(:cutover_closure_conclusion, conclusion)
      |> put_project_detail_closure_conclusion(conclusion)
    end)
  end

  defp put_project_detail_closure_conclusion(project, nil), do: project

  defp put_project_detail_closure_conclusion(project, conclusion) do
    detail =
      project
      |> value(:detail)
      |> case do
  ...
  defp cutover_authorization_consumption_guard_overview_snapshot(guard) when is_map(guard) do
    counts = value(guard, :counts) || %{}

    %{
      status: safe_status(value(guard, :status)) |> blank_to_default("no_consumption"),
      consumption_count: non_negative_integer(value(counts, :consumption_count)) || 0,
      allowed_count: non_negative_integer(value(counts, :allowed_count)) || 0,
      blocked_count: non_negative_integer(value(counts, :blocked_count)) || 0,
      no_authorization_count: non_negative_integer(value(counts, :no_authorization_count)) || 0,
      stale_count: non_negative_integer(value(counts, :stale_count)) || 0,
      manual_attention_count: non_negative_integer(value(counts, :manual_attention_count)) || 0,
      unsupported_count: non_negative_integer(value(counts, :unsupported_count)) || 0,
      malformed_count: non_negative_integer(value(counts, :malformed_count)) || 0,
      operation_decision_counts: sanitize_value(value(counts, :operation_decision_counts) || %{}),
      source_decision_counts: sanitize_value(value(counts, :source_decision_counts) || %{}),
      blocked_sources: sanitize_list(value(guard, :blocked_sources)),
      recent_decisions: sanitize_list(value(guard, :recent_decisions))
    }
  end

  defp cutover_authorization_consumption_guard_overview_snapshot(_guard), do: cutover_authorization_consumption_guard_overview_snapshot(%{})

  defp cutover_execution_outcome_ledger_overview_snapshot(ledger) when is_map(ledger) do
    counts = value(ledger, :counts) || %{}
    may_have_happened_count = non_negative_integer(value(counts, :side_effect_may_have_happened_count)) || 0

    %{
      status: safe_status(value(ledger, :status)) |> blank_to_default("no_outcome"),
      outcome_count: non_negative_integer(value(counts, :outcome_count)) || 0,
      not_executed_count: non_negative_integer(value(counts, :not_executed_count)) || 0,
      blocked_count: non_negative_integer(value(counts, :blocked_count)) || 0,
      succeeded_count: non_negative_integer(value(counts, :succeeded_count)) || 0,
      failed_count: non_negative_integer(value(counts, :failed_count)) || 0,
      retryable_count: non_negative_integer(value(counts, :retryable_count)) || 0,
      unknown_count: non_negative_integer(value(counts, :unknown_count)) || 0,
      manual_attention_count: non_negative_integer(value(counts, :manual_attention_count)) || 0,
      unsupported_count: non_negative_integer(value(counts, :unsupported_count)) || 0,
      malformed_count: non_negative_integer(value(counts, :malformed_count)) || 0,
      side_effect_entered_count: non_negative_integer(value(counts, :side_effect_entered_count)) || 0,
      side_effect_not_entered_count: non_negative_integer(value(counts, :side_effect_not_entered_count)) || 0,
      side_effect_may_have_happened_count: may_have_happened_count,
      unresolved_count: non_negative_integer(value(counts, :unresolved_count)) || 0,
      terminal_count: non_negative_integer(value(counts, :terminal_count)) || 0,
      operation_status_counts: sanitize_value(value(counts, :operation_status_counts) || %{}),
      source_status_counts: sanitize_value(value(counts, :source_status_counts) || %{}),
      guard_decision_counts: sanitize_value(value(counts, :guard_decision_counts) || %{}),
      unresolved_outcomes: sanitize_list(value(ledger, :unresolved_outcomes)),
      recent_outcomes: sanitize_list(value(ledger, :recent_outcomes))
    }
  end

  defp cutover_execution_outcome_ledger_overview_snapshot(_ledger), do: cutover_execution_outcome_ledger_overview_snapshot(%{})

  defp cutover_execution_outcome_closeout_overview_snapshot(closeout) when is_map(closeout) do
    counts = value(closeout, :counts) || %{}
    retry_consideration_count = non_negative_integer(value(counts, :allow_explicit_retry_consideration_count)) || 0

    %{
      status: safe_status(value(closeout, :status)) |> blank_to_default("no_outcome"),
      project_count: non_negative_integer(value(counts, :project_count)) || length(list_value(closeout, :projects)),
      unresolved_outcome_count: non_negative_integer(value(counts, :unresolved_outcome_count)) || 0,
      closeout_count: non_negative_integer(value(counts, :closeout_count)) || 0,
      resolved_count: non_negative_integer(value(counts, :resolved_count)) || 0,
      stale_count: non_negative_integer(value(counts, :stale_count)) || 0,
      conflict_count: non_negative_integer(value(counts, :conflict_count)) || 0,
      manual_attention_count: non_negative_integer(value(counts, :manual_attention_count)) || 0,
      malformed_count: non_negative_integer(value(counts, :malformed_count)) || 0,
      unsupported_count: non_negative_integer(value(counts, :unsupported_count)) || 0,
      no_outcome_count: non_negative_integer(value(counts, :no_outcome_count)) || 0,
      no_closeout_count: non_negative_integer(value(counts, :no_closeout_count)) || 0,
      allow_explicit_retry_consideration_count: retry_consideration_count,
      recent_reason_codes: recent_closeout_codes(closeout, :reason_code),
      recent_action_codes: recent_closeout_codes(closeout, :action_code),
      operation_status_counts: sanitize_value(value(counts, :operation_status_counts) || %{}),
      source_status_counts: sanitize_value(value(counts, :source_status_counts) || %{}),
      recent_closeouts: sanitize_list(value(closeout, :recent_closeouts)),
      auto_replay_allowed: false
    }
  end

  defp cutover_execution_outcome_closeout_overview_snapshot(_closeout), do: cutover_execution_outcome_closeout_overview_snapshot(%{})

  defp cutover_replay_decision_overview_snapshot(decision) when is_map(decision) do
    counts = value(decision, :counts) || %{}

    %{
      status: safe_status(value(decision, :status)) |> blank_to_default("no_replay_decision"),
      decision_count: non_negative_integer(value(counts, :decision_count)) || 0,
      unresolved_outcome_blocked_count: non_negative_integer(value(counts, :unresolved_outcome_blocked_count)) || 0,
      retry_consideration_allowed_count: non_negative_integer(value(counts, :retry_consideration_allowed_count)) || 0,
      retry_consideration_denied_count: non_negative_integer(value(counts, :retry_consideration_denied_count)) || 0,
      stale_closeout_count: non_negative_integer(value(counts, :stale_closeout_count)) || 0,
      conflict_count: non_negative_integer(value(counts, :conflict_count)) || 0,
      manual_attention_count: non_negative_integer(value(counts, :manual_attention_count)) || 0,
      malformed_count: non_negative_integer(value(counts, :malformed_count)) || 0,
      unsupported_count: non_negative_integer(value(counts, :unsupported_count)) || 0,
      no_unresolved_outcome_count: non_negative_integer(value(counts, :no_unresolved_outcome_count)) || 0,
      requires_operator_attention_count: non_negative_integer(value(counts, :requires_operator_attention_count)) || 0,
      recent_reason_codes: recent_replay_decision_codes(decision, :reason_code),
      recent_action_codes: recent_replay_decision_codes(decision, :action_code),
      operation_decision_counts: sanitize_value(value(counts, :operation_decision_counts) || %{}),
      source_decision_counts: sanitize_value(value(counts, :source_decision_counts) || %{}),
      recent_decisions: sanitize_list(value(decision, :recent_decisions)),
      blocked_replay: sanitize_list(value(decision, :blocked_replay)),
      auto_replay_allowed: false
    }
  end

  defp cutover_replay_decision_overview_snapshot(_decision), do: cutover_replay_decision_overview_snapshot(%{})

  defp cutover_replay_request_audit_overview_snapshot(audit) when is_map(audit) do
    counts = value(audit, :counts) || %{}
    ...
    %{
      ...
      request_count: non_negative_integer(value(counts, :request_count)) || 0,
      no_request_count: non_negative_integer(value(counts, :no_request_count)) || 0,
      allow_count: non_negative_integer(value(counts, :allow_count)) || 0,
      block_count: non_negative_integer(value(counts, :block_count)) || 0,
      stale_count: non_negative_integer(value(counts, :stale_count)) || 0,
      conflict_count: non_negative_integer(value(counts, :conflict_count)) || 0,
      manual_attention_count: non_negative_integer(value(counts, :manual_attention_count)) || 0,
      malformed_count: non_negative_integer(value(counts, :malformed_count)) || 0,
      unsupported_count: non_negative_integer(value(counts, :unsupported_count)) || 0,
      linked_outcome_recorded_count: non_negative_integer(value(counts, :linked_outcome_recorded_count)) || 0,
      linked_outcome_pending_count: non_negative_integer(value(counts, :linked_outcome_pending_count)) || 0,
      linked_outcome_blocked_count: non_negative_integer(value(counts, :linked_outcome_blocked_count)) || 0,
      linked_outcome_stale_count: non_negative_integer(value(counts, :linked_outcome_stale_count)) || 0,
      recent_reason_codes: recent_replay_request_audit_codes(audit, :reason_code),
      recent_action_codes: recent_replay_request_audit_codes(audit, :action_code),
      operation_status_counts: sanitize_value(value(counts, :operation_status_counts) || %{}),
      source_status_counts: sanitize_value(value(counts, :source_status_counts) || %{}),
      recent_requests: sanitize_list(value(audit, :recent_requests)),
      auto_replay_allowed: false
    }
  end
  ...
  defp cutover_closure_conclusion_overview_snapshot(conclusion) when is_map(conclusion) do
    %{
      conclusion: optional_string(conclusion, :conclusion) || "no_explicit_closure_chain",
      severity: optional_string(conclusion, :severity) || "none",
      attention_level: optional_string(conclusion, :attention_level) || "none",
      summary_code: optional_string(conclusion, :summary_code) || "closure_no_chain",
      required_action_codes: string_list(value(conclusion, :required_action_codes)),
      blocked_by: sanitize_list(value(conclusion, :blocked_by)),
      evidence_references: sanitize_list(value(conclusion, :evidence_references)),
      closure_chain_status: safe_status(value(conclusion, :closure_chain_status)) |> blank_to_default("no_chain"),
      closure_status_counts: sanitize_value(value(conclusion, :closure_status_counts) || %{}),
      reference_status_counts: sanitize_value(value(conclusion, :reference_status_counts) || %{}),
      project_count: non_negative_integer(value(conclusion, :project_count)) || 0,
      chain_count: non_negative_integer(value(conclusion, :chain_count)) || 0,
      fully_closed: value(conclusion, :fully_closed) == true,
      operation_success: value(conclusion, :operation_success) == true,
      read_only: value(conclusion, :read_only) != false,
      no_side_effects: value(conclusion, :no_side_effects) != false,
      actions_are_advisory: value(conclusion, :actions_are_advisory) != false,
      auto_retry_allowed: false,
      auto_replay_allowed: false,
      queued_replay: false,
      pending_execution: false,
  ...
  defp closure_conclusion_project_summary(conclusion) when is_map(conclusion) do
    %{
      ...
      provider_scope: sanitize_value(value(conclusion, :provider_scope) || %{}),
      conclusion: optional_string(conclusion, :conclusion) || "no_explicit_closure_chain",
      severity: optional_string(conclusion, :severity) || "none",
      attention_level: optional_string(conclusion, :attention_level) || "none",
      summary_code: optional_string(conclusion, :summary_code) || "closure_no_chain",
      ...
      closure_status_counts: sanitize_value(value(conclusion, :closure_status_counts) || %{}),
      reference_status_counts: sanitize_value(value(conclusion, :reference_status_counts) || %{}),
      safe_evidence_fingerprints: sanitize_value(value(conclusion, :safe_evidence_fingerprints) || %{}),
      reason_codes: string_list(value(conclusion, :reason_codes)),
      action_codes: string_list(value(conclusion, :action_codes)),
      fully_closed: value(conclusion, :fully_closed) == true,
      operation_success: value(conclusion, :operation_success) == true,
      read_only: value(conclusion, :read_only) != false,
      no_side_effects: value(conclusion, :no_side_effects) != false,
  ...
  defp closure_conclusion_detail_snapshot(conclusion) when is_map(conclusion) do
    conclusion
    |> closure_conclusion_project_summary()
    |> Map.drop([:project_id])
  end
  ...
  defp recent_replay_decision_codes(decision, key) do
    decision
  ...
  defp recent_closeout_codes(closeout, key) do
    closeout
  ...
  defp outcome_closeout_detail_snapshot(closeout) when is_map(closeout) do
    %{
      status: safe_status(value(closeout, :status)) |> blank_to_default("no_outcome"),
      counts: sanitize_value(value(closeout, :counts) || %{}),
      unresolved_outcomes: sanitize_list(value(closeout, :unresolved_outcomes)),
      closeouts: sanitize_list(value(closeout, :closeouts)),
      allow_explicit_retry_consideration: value(closeout, :allow_explicit_retry_consideration) == true,
      requires_operator_attention_reasons: string_list(value(closeout, :requires_operator_attention_reasons)),
      retry_consideration_reasons: string_list(value(closeout, :retry_consideration_reasons))
    }
  end

  defp outcome_closeout_detail_snapshot(_closeout), do: outcome_closeout_detail_snapshot(%{})

  defp replay_decision_detail_snapshot(decision) when is_map(decision) do
    %{
      status: safe_status(value(decision, :status)) |> blank_to_default("no_replay_decision"),
      counts: sanitize_value(value(decision, :counts) || %{}),
      recent_decisions: sanitize_list(value(decision, :recent_decisions)),
      blocked_replay: sanitize_list(value(decision, :blocked_replay)),
      recent_reason_codes: string_list(value(decision, :recent_reason_codes)),
      recent_action_codes: string_list(value(decision, :recent_action_codes)),
      requires_operator_attention: value(decision, :requires_operator_attention) == true,
      safe_evidence_fingerprints: sanitize_value(value(decision, :safe_evidence_fingerprints) || %{})
    }
  end

  defp replay_decision_detail_snapshot(_decision), do: replay_decision_detail_snapshot(%{})

  defp replay_request_audit_detail_snapshot(audit) when is_map(audit) do
    %{
      status: safe_status(value(audit, :status)) |> blank_to_default("no_request"),
      counts: sanitize_value(value(audit, :counts) || %{}),
      requests: sanitize_list(value(audit, :requests)),
      recent_reason_codes: string_list(value(audit, :recent_reason_codes)),
      recent_action_codes: string_list(value(audit, :recent_action_codes)),
      recent_evidence_fingerprints: sanitize_list(value(audit, :recent_evidence_fingerprints)),
      outcome_link_statuses: sanitize_value(value(audit, :outcome_link_statuses) || %{}),
      auto_replay_allowed: false
    }
  end
  ...
  defp detail_summary(
    ...
    authorization_consumption =
      authorization_consumption_detail(sources.cutover_authorization_consumption_guard, project_id)

    outcome_closeout =
      project_closeout_detail(sources.cutover_execution_outcome_closeout, project_id)

    replay_decision =
      project_replay_decision_detail(sources.cutover_replay_decision, project_id)
  ...
  defp authorization_consumption_detail(guard, project_id) do
    project = find_project(guard, project_id)

    %{
      status: safe_status(value(project || guard || %{}, :status)) |> blank_to_default("no_consumption"),
      counts: value(project || %{}, :counts) || %{},
      recent_decisions: list_value(project || %{}, :recent_decisions),
      blocked_sources: list_value(project || %{}, :blocked_sources)
    }
  end

  defp project_closeout_detail(closeout, project_id) do
    project =
      if optional_string(closeout || %{}, :project_id) == project_id do
        closeout
      else
        find_project(closeout, project_id)
      end

    %{
      status: safe_status(value(project || closeout || %{}, :status)) |> blank_to_default("no_outcome"),
      counts: value(project || %{}, :counts) || %{},
      unresolved_outcomes: list_value(project || %{}, :unresolved_outcomes),
      closeouts: list_value(project || %{}, :closeouts),
      allow_explicit_retry_consideration: value(project || %{}, :allow_explicit_retry_consideration) == true,
  ...
  defp project_replay_decision_detail(decision, project_id) do
    project =
      if optional_string(decision || %{}, :project_id) == project_id do
        decision
      else
    ...
    %{
      status: safe_status(value(project || decision || %{}, :status)) |> blank_to_default("no_replay_decision"),
      counts: value(project || %{}, :counts) || %{},
      recent_decisions: list_value(project || %{}, :recent_decisions),
      blocked_replay: list_value(project || %{}, :blocked_replay),
      recent_reason_codes: string_list(value(project || %{}, :recent_reason_codes)),
      recent_action_codes: string_list(value(project || %{}, :recent_action_codes)),
      requires_operator_attention: value(project || %{}, :requires_operator_attention) == true,
      safe_evidence_fingerprints: value(project || %{}, :safe_evidence_fingerprints) || %{}
    }
  end

  defp project_replay_request_audit_detail(audit, project_id) do
    project =
      if optional_string(audit || %{}, :project_id) == project_id do
        audit
      else
        find_project(audit, project_id)
      end

    %{
      status: safe_status(value(project || audit || %{}, :status)) |> blank_to_default("no_request"),
      counts: value(project || %{}, :counts) || %{},
      requests: list_value(project || %{}, :requests),
      recent_reason_codes: string_list(value(project || %{}, :recent_reason_codes)),
      recent_action_codes: string_list(value(project || %{}, :recent_action_codes)),
      recent_evidence_fingerprints: list_value(project || %{}, :recent_evidence_fingerprints),
