defmodule SymphonyElixir.CLITest do
  ...
  test "accepts explicit hub cutover operation request file only for hub mode" do
    parent = self()
    hub_config_path = "tmp/hub/HUB.yaml"
    request_path = "tmp/hub/cutover_request.json"
    expanded_hub_config_path = Path.expand(hub_config_path)
    expanded_request_path = Path.expand(request_path)

    deps =
      deps(%{
        file_regular?: fn path -> path in [expanded_hub_config_path, expanded_request_path] end,
        set_hub_config_path: fn _path -> :ok end,
        validate_hub_config: fn _path -> :ok end,
        load_hub_cutover_operation_request: fn path ->
          send(parent, {:hub_cutover_operation_request_loaded, path})
          :ok
        end
      })
    ...
    assert :ok =
             CLI.evaluate(
               [@ack_flag, "--hub-config", hub_config_path, "--hub-cutover-operation-request", request_path],
               deps
             )

    assert_received {:hub_cutover_operation_request_loaded, ^expanded_request_path}

    legacy_deps =
      deps(%{
        file_regular?: fn _path -> true end,
        load_hub_cutover_operation_request: fn path ->
          send(parent, {:legacy_hub_cutover_operation_request_loaded, path})
          :ok
        end
      })
  ...
  test "accepts explicit hub cutover execution authorization request file only for hub mode" do
    parent = self()
    hub_config_path = "tmp/hub/HUB.yaml"
    request_path = "tmp/hub/cutover_execution_authorization_request.json"
    expanded_hub_config_path = Path.expand(hub_config_path)
    expanded_request_path = Path.expand(request_path)
    ...
    deps =
      deps(%{
        ...
        set_hub_config_path: fn _path -> :ok end,
        validate_hub_config: fn _path -> :ok end,
        load_hub_cutover_execution_authorization_request: fn path ->
          send(parent, {:hub_cutover_execution_authorization_request_loaded, path})
          :ok
        end
      })
    ...
    assert :ok =
             CLI.evaluate(
               [
                 ...
                 "--hub-config",
                 hub_config_path,
                 "--hub-cutover-execution-authorization-request",
                 request_path
               ],
               deps
             )

    assert_received {:hub_cutover_execution_authorization_request_loaded, ^expanded_request_path}

    legacy_deps =
      deps(%{
        file_regular?: fn _path -> true end,
        load_hub_cutover_execution_authorization_request: fn path ->
          send(parent, {:legacy_hub_cutover_execution_authorization_request_loaded, path})
          :ok
        end
      })
  ...
  test "accepts explicit hub cutover replay request file only for hub mode" do
    parent = self()
    hub_config_path = "tmp/hub/HUB.yaml"
    request_path = "tmp/hub/cutover_replay_request.yaml"
    expanded_hub_config_path = Path.expand(hub_config_path)
    expanded_request_path = Path.expand(request_path)
    ...
    deps =
      deps(%{
        ...
        set_hub_config_path: fn _path -> :ok end,
        validate_hub_config: fn _path -> :ok end,
        load_hub_cutover_replay_request: fn path ->
          send(parent, {:hub_cutover_replay_request_loaded, path})
          :ok
        end
      })
    ...
    assert_received {:hub_cutover_replay_request_loaded, ^expanded_request_path}

    legacy_deps =
      deps(%{
        file_regular?: fn _path -> true end,
        load_hub_cutover_replay_request: fn path ->
          send(parent, {:legacy_hub_cutover_replay_request_loaded, path})
          :ok
        end
      })

    assert :ok = CLI.evaluate([@ack_flag, "WORKFLOW.md"], legacy_deps)
    refute_received {:legacy_hub_cutover_replay_request_loaded, _path}
  end
  ...
  test "rejects missing hub cutover operation request file" do
    hub_config_path = "tmp/hub/HUB.yaml"
    request_path = "tmp/hub/missing-request.yaml"
    expanded_hub_config_path = Path.expand(hub_config_path)
    ...
    assert {:error, message} = CLI.evaluate([@ack_flag, "--hub-config", hub_config_path, "--hub-cutover-operation-request", request_path], deps)
    assert message =~ "Hub cutover operation request file not found"
  end

  test "rejects missing hub cutover execution authorization request file" do
    hub_config_path = "tmp/hub/HUB.yaml"
    request_path = "tmp/hub/missing-authorization-request.yaml"
    expanded_hub_config_path = Path.expand(hub_config_path)
    ...
    assert {:error, message} =
             CLI.evaluate(
               [@ack_flag, "--hub-config", hub_config_path, "--hub-cutover-execution-authorization-request", request_path],
               deps
             )
  ...
  defp deps(overrides) do
    Map.merge(
      %{
        ...
        load_hub_cutover_replay_request: fn _path -> :ok end,
