...
- 可选 tracker tool 如果实现，如何限制认证来源和可访问范围，包括 GitHub/GitLab token scope、repository/project 范围、跨项目 issue link 权限和写入动作白名单。
...
- 可选工具扩展需要最小权限原则，否则会扩大 agent 对 tracker 或主机环境的影响面。GitHub/GitLab 接入尤其需要避免把组织级 token 或 group 级权限暴露给只应处理单仓库/单项目 issue 的 agent。
