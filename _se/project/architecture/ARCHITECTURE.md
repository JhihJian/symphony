...
来源：`SPEC.md`（草案 v1，语言无关）。

当前项目已有实现代码（主要在 `elixir/`），但本次架构基线尚未逐项完成代码一致性审计，本架构基线描述的是 SPEC 设定的目标状态。凡是涉及行为、模块职责和安全规则的结论，都应理解为“待实现架构要求”；只有“输入来源为 SPEC，且代码一致性尚待审计”属于本次已确认事实。

## 项目概览

Symphony 的目标是把 issue tracker 中的项目工作，稳定地交给 coding agent 执行。它不是通用工作流平台，也不是 ticket 写入系统；它的核心职责是读取可运行的工作、为每个工作准备隔离目录、启动 agent，并让 operator 能看见运行状态和失败原因。

系统边界可以用一句话概括：Symphony 负责调度、执行外壳和读取任务系统；每个 ticket 如何评论、改状态、关联 PR 或交接给人，主要由仓库中的 workflow prompt 和 agent 可用工具决定。
...
- 具体实现语言、框架或部署拓扑。
- Linear 查询字段全集或 Codex app-server 协议消息格式。
...
`工作项` 是调度单位。它来自外部 issue tracker，提供足够的信息让系统判断是否可运行、是否被阻塞、如何识别对应 workspace，以及如何向 operator 展示当前工作。

`仓库工作流` 是项目规则入口。它由仓库拥有，包含 agent 要读到的任务说明，也包含影响后续调度和运行的设置。它的意义不是替代代码实现，而是让团队把 workflow policy 随仓库版本化。
...
每轮轮询先处理正在运行的工作。调度器会刷新这些工作的外部状态，并处理长时间没有进展的 agent。如果某个工作已经终止，运行会被停止并清理 workspace；如果工作只是离开活动状态，运行会被停止但 workspace 会保留。

之后调度器读取候选工作，并在并发名额允许时派发。被派发的工作会先进入占用状态，防止同一个 issue 被重复启动。候选工作必须仍处于活动状态，不能已经运行或等待重试，也不能被仍未结束的依赖阻塞。
...
- [Issue Tracker 集成](modules/issue-tracker-integration.md)：读取外部任务系统，并输出稳定的工作项模型。
...
- ticket 写入属于 workflow 和 agent 工具边界，不进入 orchestrator 核心逻辑。
