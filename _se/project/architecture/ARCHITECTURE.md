...
当前项目已有实现代码（主要在 `elixir/`）。本架构基线描述的是 SPEC 设定的目标状态，并补充 GitHub/GitLab issue 来源接入所需的架构扩展。2026-06-22 巡检已对 GitHub/GitLab adapter、agent-side tracker tools、部分 E2E 覆盖和本地部署行为做有限代码/运行核对；其余涉及行为、模块职责和安全规则的结论仍应理解为“架构要求或待审计的实现要求”。具体证据与仍存在的漂移记录在 [来源映射](source-map.md)。
