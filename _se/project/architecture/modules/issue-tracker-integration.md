...
当前 SPEC 将 issue tracker 集成定义为 `TrackerAdapter` 边界，并覆盖 Linear、GitHub Issues 和 GitLab Issues。Linear、GitHub 和 GitLab 适配器可以改变通信细节、认证方式、分页策略、状态来源和阻塞关系读取方式，但交给 orchestrator 的工作项语义必须一致。派发策略、agent 执行、workspace 管理和 ticket 写入业务分别归属 orchestrator、Agent Runner、Workspace Manager 和 agent toolchain。
...
Linear adapter 通过 Linear GraphQL 查询读取候选、终止集合和按 ID 刷新，并把 Linear state 作为默认规范化调度状态。Linear-specific 查询、分页、认证和 payload validation 必须留在适配器内部。
