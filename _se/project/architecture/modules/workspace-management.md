...
Workspace 管理模块负责 agent 的文件系统边界。它把 issue 标识映射为稳定、安全的目录路径，确保目录存在，运行目录生命周期 hook，并在终止状态下清理目录。
...
- `Workspace Key`：由 issue identifier 安全转换得到的目录名。
...
- 将 issue identifier 转换为安全 workspace key。
